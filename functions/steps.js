...
const updateFiles = async (
  ...
  try {
    let mainfile = await data.getFileContent(
      context,
...
const nextStep = async (count, context, configyml, issueno) => {
  ...
  for (y = 0; y < configyml.steps[count].actions.length; y++) {
    ...
    if (array.type == "respond") {
      ...
      const response = await data.getFileContent(
        context,
        `.bit/responses/${responseFile}`
      );
...
const workFlow = async (context, installation) => {
  ...
  for (i = 0; i < files.data.length; i++) {
    let body = await data.getFileContent(
      context,
      `.bit/workflows/${files.data[i].name}`
    );
...
const checkIssueClosed = async (context) => {
  if (context.payload.sender.login != "counselorbot[bot]") {
    ...
    try {
      if (
        issuesComments.data.length == 0 ||
        issuesComments.data[issuesComments.data.length - 1].body !=
          "closed via oauth"
      ) {
